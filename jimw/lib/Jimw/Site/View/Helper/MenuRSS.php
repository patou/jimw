 ...
 */
class Jimw_Site_View_Helper_MenuRSS extends Zend_View_Helper_Abstract
{
    /**
     * Get the menu list
    ...
    private function getList ($name)
    {
        ...
        $select = $trees->select()->where('tree_status = 4')->where("module_path = 'blog' OR module_path = 'categoryblog'")->order('tree_lft ASC');
