...
class FileUpload {
	...
	function run() {
		if ((sizeof($_FILES)==0)&&(is_array($_GET['file']['NewFile']))) {
				...
				if ($_FILES['NewFile']['size']<($typeconfig['MaxSize']*1024)) {
					...
					if ($lastdot!==false) {
						...
						if (in_array(strtolower($ext),$typeconfig['AllowedExtensions'])) {
							...
							if ((($this->fckphp_config['DiskQuota']['Global']!=-1)||($this->fckphp_config['DiskQuota'][$this->type]!=-1))&&$failSizeCheck) {
								//Disk Quota over
										...
										if (!file_exists($this->real_cwd."/$filename($i).$ext")) {
											if (is_uploaded_file($_FILES['NewFile']['tmp_name'])) {
												...
												(move_uploaded_file($_FILES['NewFile']['tmp_name'],($this->real_cwd."/$filename($i).$ext"))) {
													...
													$disp="202,'Failed to upload file, internal error.'".$this->real_cwd."/$filename($i).$ext";
												}
											} else {
												if 
