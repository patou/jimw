 ...
 * @version    $Id: MessageController.php 352 2009-09-06 18:09:49Z patou.de.saint.steban $
 */
include_once (dirname(__FILE__) . '/../../Controller/Model/ShopAlbum.php');
include_once (dirname(__FILE__) . '/../../Controller/Model/ShopSong.php');
include_once (dirname(__FILE__) . '/../Form/AlbumForm.php');
class Shop_AlbumController extends Jimw_Admin_Action
{
	public function listAction () {
		$albummodel = new ShopAlbum();
		$request = $this->getRequest();
		$id = $request->getParam('id');
		$result = Zend_Paginator::factory($albummodel->select()->where('tree_id = ?', $id)->order('shopalbum_id'));
		...
		$this->view->albums_list = $result;
		$this->view->tree_id = $id;
	}
